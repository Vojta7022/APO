...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < width; i++)
        histogram[(int)round(0.2126 * prev_row[i * COLORS] +
                             0.7152 * prev_row[i * COLORS + 1] +
                             0.0722 * prev_row[i * COLORS + 2]) /
                  51]++;
    ...
    for (int i = 1; i < height - 1; i++)
    {
        ...
        histogram[(int)round(0.2126 * curr_row[0] +
                             0.7152 * curr_row[1] +
                             0.0722 * curr_row[2]) /
                  51]++;
        ...
        histogram[(int)round(0.2126 * curr_row[(width - 1) * COLORS] +
                             0.7152 * curr_row[(width - 1) * COLORS + 1] +
                             0.0722 * curr_row[(width - 1) * COLORS + 2]) /
                  51]++;
    ...
    for (int i = 0; i < width; i++)
        histogram[(int)round(0.2126 * curr_row[i * COLORS] +
                             0.7152 * curr_row[i * COLORS + 1] +
                             0.0722 * curr_row[i * COLORS + 2]) /
                  51]++;
